...
acuteag_dir="${current_dir}/../AcuteagBase"
acuteag_release_dir="${SOURCE_SERVER}:/qcom/AcuteagBase"
acuteag_release_branch="master"
acuteag_master_branch="master"
...
release_createnotes()
{
    ...
    echo -e "Apk Url: \\192.168.31.242/rom-release/APP_Release/${module_name}/${branch_name}/${apk_version}\n" >> ReleaseNotes.txt
...
release_upload_sw()
{
   ...
   scp -r ${prefix}/* root@192.168.31.242:/home/samba/rom/APP_Release/${module_name}/${branch_name}/${apk_version}   
  trap - ERR
}
...
release_sendemail()
{
    ...
    if [ "$MAIL_LIST" == "" ]; then
        ...
        echo "Please contact with integration@acuteangle.com"
        echo "******************************************************"
        exit 1
    fi
    ...
    if [ -f ${current_dir}/int/common/acuteagapps_release_cc.list  ]; then
        ...
        MAIL_LIST_cc="integration@acuteangle.com"
    fi       
