...
project_remote="/home/samba/rom/APP_Release"
...
#bye
#EOF
scp -r root@192.168.31.242:${project_remote}/${project_sync}/${releasedir}/$versionsync ./
echo "scp -r ${project_remote}/${project_sync}/${releasedir}/$versionsync ./"
...
if [ ! -d ${acuteag_release_dir}/vendor/acuteangle/acuteag-apps/${project_sync} ];then
    echo "****** Error:${acuteag_release_dir}/vendor/acuteangle/acuteag-apps/${project_sync} has not been set,Pls contacts Admin!"
    exit 1
fi
...
old_module_path=`ls ${acuteag_release_dir}/vendor/acuteangle/acuteag-apps/${project_sync}/*.apk`
old_module_name=`basename ${old_module_path}`
echo "****** Old_module_version:${old_module_name}"

echo "****** Cp ${versionsync}/${apknamesigned} ${acuteag_release_dir}/vendor/acuteangle/acuteag-apps/${project_sync}/${out_module_name}"

cp ${versionsync}/${apknamesigned} ${acuteag_release_dir}/vendor/acuteangle/acuteag-apps/${project_sync}/${out_module_name}.apk
...
if ( ! ((git status | grep "nothing to commit") || (git status | grep "无文件要提交") ));then
    ...
    git pull
    rm vendor/acuteangle/acuteag-apps/${project_sync}/${old_module_name}
    echo "zt:git rm vendor/acuteangle/acuteag-apps/${project_sync}/${old_module_name}"
    echo "remove old apk"
    git add -A
