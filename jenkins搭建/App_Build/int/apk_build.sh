...
upload_sw()
{
   ...
   # ${scripts_dir}/upload_make.exp $module_name "${release_dir}"  $prefix ${apk_version} $PASS
    ssh root@192.168.31.242  "[ -d /home/samba/rom/APP_Release/$module_name/${branch_name}/${apk_version} ] && echo ok || mkdir -p /home/samba/rom/APP_Release/$module_name/${branch_name}/${apk_version}" 
    scp -r  * root@192.168.31.242:/home/samba/rom/APP_Release/$module_name/${branch_name}/${apk_version}/

    echo "*****************************************Info********************************************"
    echo "Please download apk from SMB 192.168.31.242 /APP_Release/$module_name/${branch_name}/${apk_version}/"
    echo "*****************************************************************************************"
}
...
update_aa_base_code() {
    if [ "${build_env}" == "msm8998" ] ; then
      ...
      cd ${current_dir}/msm8998     
      if [ -d AcuteagBase ];then
       rm AcuteagBase -rf
      fi
      repo init -u gerritroot@192.168.31.242:manifest -m  acuteag-msm8998_r1.xml -b master
      echo "******* Start repo forall -c 'git clean -df && git reset --hard HEAD'"
      repo forall -c 'git clean -df && git reset --hard HEAD'
      repo sync -j16
      repo start --all master      

      if [ -d vendor/acuteangle/acuteag-framework ];then
           rm vendor/acuteangle/acuteag-framework/ -rf
      fi
         cd ../../acuteag-framework
         git clean -df
         git pull
         cd ${current_dir}/msm8998
         cp ../../acuteag-framework vendor/acuteangle/ -rf
         echo "cp ../../acuteag-framework vendor/acuteangle/"
      fi
}
