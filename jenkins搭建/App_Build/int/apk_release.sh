...
acuteag_release_dir="${SOURCE_SERVER}:/eastaeon/acuteag"
acuteag_release_branch="acuteangle_k31_dev"
acuteag_master_branch="acuteangle_k31_1.0.1_master"
...
release_checkpara()
{
    ...
    if [ ${build_env} == "acuteangle" ];then
        ...
        if [ -f ${release_checkfolder}/${branch_name}/build.gradle ]; then
            var=`sed -n 's/.*versionName *\"\(.*\)\".*/\1/p' ${release_checkfolder}/${branch_name}/build.gradle | grep -o "[0-9][0-9a-z.]*"`
            vercodetmp=`sed -n 's/.*versionCode *\(.*\)/\1/p' ${release_checkfolder}/${branch_name}/build.gradle | grep -o "[0-9][0-9a-z.]*"`
        else
            echo "*********************Error**********************************"
...
release_createnotes()
{
    ...
    echo -e "Apk Url: ftp://hmb-release@192.168.1.108/APP_Release/${module_name}/${branch_name}/${apk_version}\n" >> ReleaseNotes.txt
...
release_syncapkrele()
{
    ...
    if [ "${branch_name}" == "hmb_1.0.1_dev" ];then
        acuteag_release_branch="acuteangle_k31_dev"
    elif [ "${branch_name}" == "hmb_1.0.1_master" ];then
        acuteag_release_branch="acuteangle_k31_1.0.1_master"
    fi
...
release_sendemail()
{
    ...
    if [ -f ${current_dir}/../acuteag_generaltools/maillist/${git_store_name}.list ]; then
        while read mailname;
        do
            echo "******* mailname:${mailname}"
            MAIL_LIST="${MAIL_LIST},$mailname"
        done < ${current_dir}/../acuteag_generaltools/maillist/${git_store_name}.list
    fi
